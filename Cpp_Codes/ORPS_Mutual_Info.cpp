...
# define nmessgs 3000                  // total number of messages
# define npacks 40                    // no of packets per message 
# define no_nodes 6                  // no of nodes in network 
//# define M_max 5                      // max no of lost packs
# define n_Tx 300                     // max no of additional retrans
# define nlinks 18 //always                    // no of links in network 
...
int main()
{
	...
	{	
        ...
        for (int i=0; i<no_nodes; ++i)
        {  rec_vect[i] = 0;         // all nodes have zero packets %%%%% NEEDS TO BE CHANGED -- DONE
		...
		{  for (int j=0; j<npacks; ++j)
		     for (int k=0; k<no_nodes; ++k)
			     rec_matrix[i][j][k] = false;  // all nodes have empty buckets %%%%% NEEDS TO BE CHANGED -- DONE
		}		
            ...
            {  ntrans++;
               ...
               {  int f = nxtfwdset[i];
				  ...
				  {	  int found = 0;
					  ...
					  if (!found)
					  { rec_vect[f] = rec_vect[f] + 1;      // %%%%% BLOCK NEEDS TO BE CHANGED -- DONE
						int j = rec_vect[f];
						if (j==npacks)
						{   ACK[fwd]=true;
							ackfwd[f] = true;
            ...
            {  ackmincost = 9999;
               ...
               {  int f = nxtfwdset[i];
                  ...
                  {  float linccost = mean_link_cost[fwd][f];
					 float addcost = addnl_Tx_est(ntrans-npacks,pe[fwd][f],ntrans, linccost);    // addnl retrans others need to receive the message
				     // %%%%% NEEDS TO BE CHANGED -- DONE
            ...
            {  ntrans = ntrans + 1;
               ...
               {  ackmincost = 9999;
                  ...
                  {  int f = nxtfwdset[i];
                     ...
                     {  float linccost = mean_link_cost[fwd][f];
						float addcost = addnl_Tx_est(ntrans-npacks,pe[fwd][f],ntrans, linccost);
						if (mean_cost[f] + addcost < nackmincost)
                        nackmincost = mean_cost[f] + addcost;
                     }                    
                  }
...
float addnl_Tx_est(int L, float p, int ntrans, float linccost)
{ /*

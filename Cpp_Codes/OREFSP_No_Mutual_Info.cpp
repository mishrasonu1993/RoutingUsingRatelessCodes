...
# define npacks 5                     // no of packets per message 
# define no_nodes 5                   // no of nodes in network 
# define M_max 5                      // max no of lost packs
# define n_Tx 120                       // max no of additional retrans
# define nlinks 6                     // no of links in network 
...
//	tlr_coeffs_read();                  // read taylor coefficients from a file
	...
	{	
		...
		{
			for (int i=0; i<no_nodes; ++i)
            {  rec_vect[i] = 0;         // all nodes have zero packets %%%%% NEEDS TO BE CHANGED
               ackfwd[i] = false;       // none of the nodes have sent any ACK
		       nxtfwdset[i] = 0;        // downstream forwarders arn't chosen yet
		    ...
		    {  for (int j=0; j<npacks; ++j)
			      for (int k=0; k<no_nodes; ++k)
				     rec_matrix[i][j][k] = false;  // all nodes have empty buckets %%%%% NEEDS TO BE CHANGED
		    }
			 
            int j = 0;
            for (int i=0; i<no_nodes; ++i)  // selecting the potential forwarders
            {  if (i==fwd) continue;
               if (pe[fwd][i] < 0.95 && mean_cost[i] < mean_cost[fwd])
               { nxtfwdset[j]=i;          
                 ++j;
               }               
            }        
            ...
            {  ntrans++;
               ...
               {  int f = nxtfwdset[i];
                  ...
                  if (!r)
                  {  rec_vect[f] = rec_vect[f] + 1;      // %%%%% BLOCK NEEDS TO BE CHANGED
                     j = rec_vect[f];
                     if (j==npacks)
                     {  ACK[fwd]=true;
                        ackfwd[f] = true;
            ...
            {  ackmincost = 9999;
               ...
               {  int f = nxtfwdset[i];
                  ...
                  {  float addcost = addnl_Tx_est(rec_vect[f],pe[fwd][f]);    // addnl retrans others need to receive the message
				     // %%%%% NEEDS TO BE CHANGED
            ...
            {  ntrans = ntrans + 1;
               ...
               {  int f = nxtfwdset[i];
                  ...
                  int r = sto.Binomial(1, pe[fwd][f]);  // sample the bi
               ...
               {  ackmincost = 9999;
                  ...
                  {  int f = nxtfwdset[i];
                     ...
                     {  float addcost = addnl_Tx_est(rec_vect[f],pe[fwd][f]);
						if (mean_cost[f] + addcost < nackmincost)
                        nackmincost = mean_cost[f] + addcost;
                     }                    
                  }
...
{ /*
  ...
  */
  ...
  { float min_cost = 9999;
    for (int i=0; i<no_nodes; ++i)
    { if (pe[node][i]<0.95 && mean_cost[i]<mean_cost[node])
      { int dwnode = i;
...
float addnl_Tx_est(int K, float p)
{ /*
